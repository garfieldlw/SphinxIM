...
class DaoDictEn2 {
    ...
    static let shared = DaoDictEn2()
    
    private let table = Table("dict_en_2")
    ...
    func List(id: Int64, code: String, limit: Int) -> [EntityEn2Dict] {
        ...
        var dicts:[EntityEn2Dict] = []
        do {
            
            let ds = try Dao.shared.db?.prepare(query)
            for d in ds! {
                dicts.append( EntityEn2Dict(id:d[cId], code:d[cCode], text:d[cText], count:d[cCount], create:d[cCreate]))
            }
            
        }catch {
            NSLog("[DaoDictEn2]list: "+error.localizedDescription)
        }
